...
public class TokenProcessingService {
    ...
    public EmployeeEntity authenticateRequest(HttpServletRequest request) throws JsonParseException, JsonMappingException, IOException
    {
        final String token = request.getHeader(AUTH_HEADER_NAME);
        System.out.println("TOKEN: " + token );
        if(token == null)
            return null;
        EmployeeEntity user = tokenHandler.processToken(token);
        if(user == null)
            return null;
        return user;
    }
